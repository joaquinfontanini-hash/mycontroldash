...
- **Core Features:**
    ...
    - **Finanzas Personales — Fase 1 + Fase 2 (completo):**
      ...
      - Frontend (finance.tsx): 6 tabs — Resumen (PressureMeter + compromisos card + 6 KPI cards + upcoming payments + alerts + recent tx + upcoming recurrences + category breakdown + accounts mini), Movimientos (table with filters + cardId filter), Cuentas (card grid), Tarjetas (color-coded cards + installment plans per card), Prestamos (total adeudado + progress bars + next due), Recurrencias (active/paused with inline pause/reactivate buttons).
      - New modals: CardModal (color picker, closeDay/dueDay), InstallmentPlanModal (auto-calc on blur), LoanModal (status: active/paid/defaulted).
