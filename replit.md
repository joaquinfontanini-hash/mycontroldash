...
- **Core Features:**
    ...
    - **Presupuestos y Cobranzas** (module key: `quotes`, route: `/dashboard/quotes`): Full billing/invoicing module. Two quote types: `single` (classic) and `recurring_indexed` (recurring contracts with IPC/ICL index adjustment). DB tables: `quotes`, `quote_items`, `quote_revisions`, `quote_payments`, `quote_activity_logs`, `quote_installments`, `quote_adjustments`. Key features: auto-generate installments from contract dates + billing frequency; `POST /quotes/:id/apply-adjustment` applies IPC rate only to future pending installments (never overdue/paid/partially_paid); `VALID_TRANSITIONS` state machine enforces status flow; `recalcStatus` handles approved→expired; extended KPIs (contratosActivos, cuotasPendientes, cuotasVencidas, ingresosProyMes, proximoAjuste). Frontend: QuoteForm with type toggle, InstallmentsTab with per-cuota payment modal, AdjustmentsTab with IPC history, KPI row for recurring contracts, quoteType filter in list. **Client integration**: `ClientQuotesSummary` in clients.tsx shows 6 KPIs (presupuestado, cobrado, saldo, vencidos, parciales, último pago) + 3 tabs: Presupuestos / Cobros / Cuotas (visible only when recurring installments exist). `GET /api/quotes/client/:id` returns installments list. **Studio datasets** (Finance integration): 4 new datasets: `quotes.projected.monthly` (future pending installments grouped by month, configurable horizon), `quotes.outstanding.receivables` (total outstanding across quotes+installments), `quotes.installments.future` (upcoming cuotas next N days), `quotes.installments.overdue` (cuotas vencidas with client name). Total studio datasets: 9 (was 5). **Spec compliance**: integración con clientes ✅, tabla de cuotas en cliente ✅, datasets projected_revenue_by_month/outstanding_receivables/future_installments/overdue_installments ✅, fuente única (Finanzas consume, no duplica) ✅.
