...
// ── Clerk env vars (solo en modo Clerk) ───────────────────────────────────────
const clerkPubKey   = import.meta.env["VITE_CLERK_PUBLISHABLE_KEY"] as string | undefined;
const clerkProxyUrl = import.meta.env["VITE_CLERK_PROXY_URL"] as string | undefined;

if (!LOCAL_AUTH_MODE && !clerkPubKey) {
  throw new Error("VITE_CLERK_PUBLISHABLE_KEY falta en el archivo .env");
}
...
// ── Componentes de página (definidos a nivel módulo → se crean una sola vez) ──
// NO definirlos dentro de funciones de render — cada render recrearía el componente,
// causando unmount/mount completo de la página al navegar.
...
// Extraída fuera del render del Switch — el original la definía inline como
// () => <Suspense>..., creando un nuevo componente en cada render del Switch
const Notifications    = () => <Suspense fallback={null}><NotificationsPage /></Suspense>;
...
// Extraído como componente separado para no duplicar entre LocalApp y ClerkApp
function AppRoutes({ isLocal = false }: { isLocal?: boolean }) {
  return (
    <Switch>
      {/* Rutas públicas */}
      {isLocal
      ...
      {!isLocal && <Route path="/sign-up/*?" component={SignUpPage} />}
...
function LocalAuthBridge({ children }: { children: React.ReactNode }) {
  ...
  const value           = buildAuthValue(
    ...
    undefined,
    () => { clearLocalSession(); setLocation("/sign-in"); },
  );
