import { Router, type IRouter, type Request } from "express";
import { eq, desc, and } from "drizzle-orm";
import { z } from "zod";
import { db, usersTable } from "@workspace/db";
import { UpdateUserBody, UpdateUserParams } from "@workspace/api-zod";
import { getAuth } from "@clerk/express";
import {
  requireAuth,
  requireAdmin,
  requireSuperAdmin,
  type AuthenticatedRequest,
} from "../middleware/require-auth.js";
import { logSecurityEvent, getClientIp } from "../lib/security-logger.js";
import { logger } from "../lib/logger.js";

const router: IRouter = Router();

// ── Helper: sanitizar usuario para response ───────────────────────────────────
// Igual que en auth.ts — NO exponer passwordHash ni campos internos.
// Los admins reciben algunos campos extra (isBlocked, blockedReason) que los
// usuarios normales no necesitan ver de otros usuarios.
function sanitizeUserPublic(user: typeof usersTable.$inferSelect) {
  return {
    id:       user.id,
    name:     user.name,
    email:    user.email,
    role:     user.role,
    isActive: user.isActive,
    clerkId:  user.clerkId,
  };
}

function sanitizeUserAdmin(user: typeof usersTable.$inferSelect) {
  return {
    id:                 user.id,
    name:               user.name,
    email:              user.email,
    role:               user.role,
    isActive:           user.isActive,
    isBlocked:          user.isBlocked,
    blockedAt:          user.blockedAt,
    blockedReason:      user.blockedReason,
    mustChangePassword: user.mustChangePassword,
    lastActivityAt:     user.lastActivityAt,
    clerkId:            user.clerkId,
    createdAt:          user.createdAt,
    updatedAt:          user.updatedAt,
  };
}

function parseId(raw: unknown): number | null {
  const n = Number(raw);
  return Number.isInteger(n) && n > 0 ? n : null;
}

// ── GET /users/assignable ─────────────────────────────────────────────────────
// Lista mínima de usuarios activos para dropdowns de asignación (tareas, etc.)
// Solo expone id, name, email — sin campos sensibles.
router.get("/users/assignable", requireAuth, async (_req, res): Promise<void> => {
  try {
    const users = await db
      .select({
        id:    usersTable.id,
        name:  usersTable.name,
        email: usersTable.email,
      })
      .from(usersTable)
      .where(and(eq(usersTable.isActive, true), eq(usersTable.isBlocked, false)))
      .orderBy(usersTable.name);
    res.json(users);
  } catch (err) {
    logger.error({ err }, "users/assignable error");
    res.status(500).json({ error: "Error al cargar usuarios" });
  }
});

// ── GET /users/me ─────────────────────────────────────────────────────────────
// Devuelve el perfil del usuario autenticado. Soporta sesión local y Clerk.
// NUNCA expone passwordHash.
router.get("/users/me", async (req, res): Promise<void> => {
  try {
    const sessionUserId = req.session?.userId;
    if (sessionUserId) {
      const [user] = await db
        .select()
        .from(usersTable)
        .where(eq(usersTable.id, sessionUserId));
      if (!user || !user.isActive || user.isBlocked) {
        res.status(401).json({ error: "Unauthorized" });
        return;
      }
      res.json(sanitizeUserAdmin(user));
      return;
    }

    const clerkId = getAuth(req)?.userId;
    if (!clerkId) {
      res.status(401).json({ error: "Unauthorized" });
      return;
    }

    const [user] = await db
      .select()
      .from(usersTable)
      .where(eq(usersTable.clerkId, clerkId));
    if (!user) {
      res.status(404).json({ error: "User not found" });
      return;
    }
    res.json(sanitizeUserAdmin(user));
  } catch (err) {
    logger.error({ err }, "users/me error");
    res.status(500).json({ error: "Error interno" });
  }
});

// ── GET /users ────────────────────────────────────────────────────────────────
// Solo admins. Devuelve todos los usuarios con campos admin completos.
router.get("/users", requireAdmin, async (_req, res): Promise<void> => {
  try {
    const users = await db
      .select()
      .from(usersTable)
      .orderBy(desc(usersTable.createdAt));
    // Admins ven la lista completa pero sin passwordHash
    res.json(users.map(sanitizeUserAdmin));
  } catch (err) {
    logger.error({ err }, "users list error");
    res.status(500).json({ error: "Error al cargar usuarios" });
  }
});

// ── POST /users ───────────────────────────────────────────────────────────────
// Registro de usuario desde Clerk webhook o flujo de onboarding.
// Protegido: solo puede llamarse desde una sesión Clerk válida (el usuario
// se está registrando con su propio token) o con requireAuth para el admin.
// El original era completamente público — cualquiera podía crear usuarios.
router.post("/users", async (req, res): Promise<void> => {
  // Verificar que la request tiene un token Clerk válido
  const clerkId = getAuth(req)?.userId;
  if (!clerkId) {
    res.status(401).json({ error: "Token Clerk requerido para registrar usuario" });
    return;
  }

  const schema = z.object({
    clerkId: z.string().min(1),
    email:   z.string().email(),
    name:    z.string().optional().nullable(),
  });

  const parsed = schema.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.issues[0]?.message ?? "Datos inválidos" });
    return;
  }

  // El token Clerk en la request debe corresponder al clerkId del body
  if (clerkId !== parsed.data.clerkId) {
    logger.warn(
      { tokenClerkId: clerkId, bodyClerkId: parsed.data.clerkId },
      "POST /users: token clerkId mismatch",
    );
    res.status(403).json({ error: "No autorizado" });
    return;
  }

  try {
    const [existing] = await db
      .select()
      .from(usersTable)
      .where(eq(usersTable.clerkId, parsed.data.clerkId));
    if (existing) {
      res.json(sanitizeUserAdmin(existing));
      return;
    }

    const superAdminEmail = process.env["SUPER_ADMIN_EMAIL"];
    const role =
      superAdminEmail && parsed.data.email === superAdminEmail
        ? "super_admin"
        : "viewer";

    const [user] = await db
      .insert(usersTable)
      .values({
        clerkId: parsed.data.clerkId,
        email:   parsed.data.email,
        name:    parsed.data.name ?? null,
        role,
      })
      .returning();

    await logSecurityEvent({
      actorClerkId: parsed.data.clerkId,
      actorEmail:   parsed.data.email,
      action:       "user_registered",
      result:       "success",
      metadata:     { role },
    });

    res.status(201).json(sanitizeUserAdmin(user));
  } catch (err) {
    logger.error({ err }, "user registration error");
    res.status(500).json({ error: "Error al registrar usuario" });
  }
});

// ── PATCH /users/:id ─────────────────────────────────────────────────────────
// Solo admins. Actualiza nombre, rol y campos permitidos.
// No permite que un admin se promueva a super_admin a sí mismo.
router.patch("/users/:id", requireAdmin, async (req: Request, res): Promise<void> => {
  try {
    const params = UpdateUserParams.safeParse(req.params);
    if (!params.success) {
      res.status(400).json({ error: params.error.issues[0]?.message ?? "ID inválido" });
      return;
    }

    const parsed = UpdateUserBody.safeParse(req.body);
    if (!parsed.success) {
      res.status(400).json({ error: parsed.error.issues[0]?.message ?? "Datos inválidos" });
      return;
    }

    const actor  = (req as AuthenticatedRequest).dbUser;
    const [target] = await db
      .select()
      .from(usersTable)
      .where(eq(usersTable.id, params.data.id));

    if (!target) {
      res.status(404).json({ error: "Usuario no encontrado" });
      return;
    }

    // Protección: solo super_admin puede modificar a otro super_admin
    if (target.role === "super_admin" && actor?.role !== "super_admin") {
      res.status(403).json({ error: "Solo el super_admin puede modificar a otro super_admin" });
      return;
    }

    // Protección: solo super_admin puede otorgar el rol super_admin
    // (parsed.data as any) eliminado — accedemos al tipo correctamente
    const newRole = (parsed.data as { role?: string }).role;
    if (newRole === "super_admin" && actor?.role !== "super_admin") {
      res.status(403).json({ error: "Solo el super_admin puede otorgar ese rol" });
      return;
    }

    const [user] = await db
      .update(usersTable)
      .set(parsed.data)
      .where(eq(usersTable.id, params.data.id))
      .returning();

    if (!user) {
      res.status(404).json({ error: "Usuario no encontrado" });
      return;
    }

    await logSecurityEvent({
      actorClerkId:  actor?.clerkId ?? null,
      actorEmail:    actor?.email ?? null,
      targetClerkId: target.clerkId,
      targetEmail:   target.email,
      action:        "user_updated",
      result:        "success",
      metadata:      { changes: parsed.data },
      ipAddress:     getClientIp(req),
    });

    res.json(sanitizeUserAdmin(user));
  } catch (err) {
    logger.error({ err }, "users/update error");
    res.status(500).json({ error: "Error al actualizar usuario" });
  }
});

// ── POST /users/:id/block ─────────────────────────────────────────────────────
router.post("/users/:id/block", requireAdmin, async (req: Request, res): Promise<void> => {
  try {
    const id = parseId(req.params["id"]);
    if (!id) { res.status(400).json({ error: "ID inválido" }); return; }

    const actor  = (req as AuthenticatedRequest).dbUser;
    const reason = typeof req.body?.reason === "string" ? req.body.reason : null;

    const [target] = await db
      .select()
      .from(usersTable)
      .where(eq(usersTable.id, id));
    if (!target) { res.status(404).json({ error: "Usuario no encontrado" }); return; }

    if (target.role === "super_admin") {
      res.status(403).json({ error: "No se puede bloquear al super_admin" });
      return;
    }
    if (actor?.id === id) {
      res.status(403).json({ error: "No podés bloquearte a vos mismo" });
      return;
    }

    const [updated] = await db
      .update(usersTable)
      .set({ isBlocked: true, blockedAt: new Date(), blockedReason: reason })
      .where(eq(usersTable.id, id))
      .returning();

    await logSecurityEvent({
      actorClerkId:  actor?.clerkId ?? null,
      actorEmail:    actor?.email ?? null,
      targetClerkId: target.clerkId,
      targetEmail:   target.email,
      action:        "user_blocked",
      result:        "success",
      metadata:      { reason },
      ipAddress:     getClientIp(req),
    });

    res.json(sanitizeUserAdmin(updated));
  } catch (err) {
    logger.error({ err }, "block user error");
    res.status(500).json({ error: "Error al bloquear usuario" });
  }
});

// ── POST /users/:id/unblock ───────────────────────────────────────────────────
router.post("/users/:id/unblock", requireAdmin, async (req: Request, res): Promise<void> => {
  try {
    const id = parseId(req.params["id"]);
    if (!id) { res.status(400).json({ error: "ID inválido" }); return; }

    const actor = (req as AuthenticatedRequest).dbUser;

    const [target] = await db
      .select()
      .from(usersTable)
      .where(eq(usersTable.id, id));
    if (!target) { res.status(404).json({ error: "Usuario no encontrado" }); return; }

    const [updated] = await db
      .update(usersTable)
      .set({ isBlocked: false, blockedAt: null, blockedReason: null })
      .where(eq(usersTable.id, id))
      .returning();

    await logSecurityEvent({
      actorClerkId:  actor?.clerkId ?? null,
      actorEmail:    actor?.email ?? null,
      targetClerkId: target.clerkId,
      targetEmail:   target.email,
      action:        "user_unblocked",
      result:        "success",
      ipAddress:     getClientIp(req),
    });

    res.json(sanitizeUserAdmin(updated));
  } catch (err) {
    logger.error({ err }, "unblock user error");
    res.status(500).json({ error: "Error al desbloquear usuario" });
  }
});

// ── POST /users/:id/promote-super-admin ───────────────────────────────────────
// Solo super_admin puede promover a otro usuario a super_admin.
router.post("/users/:id/promote-super-admin", requireSuperAdmin, async (req: Request, res): Promise<void> => {
  try {
    const id = parseId(req.params["id"]);
    if (!id) { res.status(400).json({ error: "ID inválido" }); return; }

    const actor = (req as AuthenticatedRequest).dbUser;

    const [existing] = await db
      .select()
      .from(usersTable)
      .where(eq(usersTable.id, id));
    if (!existing) { res.status(404).json({ error: "Usuario no encontrado" }); return; }

    const [updated] = await db
      .update(usersTable)
      .set({ role: "super_admin" })
      .where(eq(usersTable.id, id))
      .returning();

    await logSecurityEvent({
      actorClerkId:  actor?.clerkId ?? null,
      actorEmail:    actor?.email ?? null,
      targetClerkId: existing.clerkId,
      targetEmail:   existing.email,
      action:        "user_promoted_super_admin",
      result:        "success",
      ipAddress:     getClientIp(req),
    });

    res.json(sanitizeUserAdmin(updated));
  } catch (err) {
    logger.error({ err }, "promote super admin error");
    res.status(500).json({ error: "Error al promover usuario" });
  }
});

export default router;
