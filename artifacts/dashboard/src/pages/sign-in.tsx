...
function LocalPasswordForm({ onSuccess }: { onSuccess: (mustChangePassword: boolean) => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (LOCAL_AUTH_MODE) {
        const valid = verifyLocalPassword(password);
        if (!valid) {
    ...
    <div className="min-h-[100dvh] flex items-center justify-center bg-background p-4">
      <div className="w-full max-w-sm space-y-6 rounded-xl border bg-card p-8 shadow-sm">
        <div className="space-y-1 text-center">
          ...
          <p className="text-sm text-muted-foreground">Panel de gestión profesional</p>
        </div>
          ...
          <>
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-card px-2 text-muted-foreground">o ingresá con contraseña</span>
              </div>
            </div>
          </>
        )}
