/**
 * Studio Generation Engine v2
 * Heuristic prompt parser — no LLM required.
 * Produces a valid dashboard config from free-text, template key, or wizard input.
 *
 * IMPORTANT: buildDefaultLayouts() must be called AFTER widget DB insert so it
 * can reference actual widget IDs. Pass the returned DB rows.
 */
...
export interface LayoutItem {
  widgetId: number;   // actual DB id — stable reference
  x: number;
  y: number;
  w: number;
  h: number;
...
function widgetsForDomain(domain: string, secondary: string[]): SuggestedWidget[] {
  ...
  if (all.includes("tareas")) {
    ...
    add({ type: "ranking",        title: "Panel del Equipo",         dataSourceKey: "tasks.teamBoard", configJson: {} });
...
export function generateFromPrompt(prompt: string): GeneratedDashboard {
  ...
  return {
    ...
    category: domain,
    widgets: widgetsForDomain(domain, secondaryDomains),
    filters: filtersForDomain(domain),
    parsedIntent: intent,
  };
}
...
export function generateFromTemplate(
  ...
  templateMeta?: { name?: string; icon?: string; color?: string; category?: string; description?: string }
): GeneratedDashboard {
  const domain = templateMeta?.category ?? "general";
  return {
    name:        templateMeta?.name        ?? "Dashboard desde plantilla",
    description: templateMeta?.description ?? "Dashboard creado a partir de una plantilla.",
    icon:        templateMeta?.icon        ?? DOMAIN_ICONS[domain]  ?? DOMAIN_ICONS.default,
    color:       templateMeta?.color       ?? DOMAIN_COLORS[domain] ?? DOMAIN_COLORS.default,
    category:    domain,
...
export function generateFromWizard(input: WizardInput): GeneratedDashboard {
  return {
    ...
    icon:        input.icon  ?? DOMAIN_ICONS[input.category]  ?? DOMAIN_ICONS.default,
    color:       input.color ?? DOMAIN_COLORS[input.category] ?? DOMAIN_COLORS.default,
...
// ── Layout generator ──────────────────────────────────────────────────────────
// MUST be called after widget rows are inserted; pass the DB-returned rows with IDs.

export function buildDefaultLayouts(
  dbWidgets: Array<{ id: number; orderIndex?: number | null }>
): { desktop: LayoutItem[]; mobile: LayoutItem[] } {
  const sorted = [...dbWidgets].sort((a, b) => (a.orderIndex ?? 0) - (b.orderIndex ?? 0));

  const desktop: LayoutItem[] = sorted.map((w, i) => ({
    widgetId: w.id,
    x: (i % 3) * 4,
    y: Math.floor(i / 3) * 4,
    w: 4,
  ...
  const mobile: LayoutItem[] = sorted.map((w, i) => ({
    widgetId: w.id,
    x: 0,
    y: i * 5,
    w: 12,
