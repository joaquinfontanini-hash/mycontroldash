...
function addMonths(dateStr: string, n: number) {
  const d = new Date(dateStr + "T12:00:00Z"); d.setUTCMonth(d.getUTCMonth() + n); return d.toISOString().slice(0, 10);
}

async function ensureDefaultCategories(userId: string) {
  const existing = await db.select().from(financeCategoriesTable).where(eq(financeCategoriesTable.userId, userId));
  if (existing.length > 0) return;
  const defaults = [...DEFAULT_INCOME_CATEGORIES, ...DEFAULT_EXPENSE_CATEGORIES].map(c => ({ ...c, userId, isDefault: true }));
...
router.get("/finance/categories", requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json(cats);
  } catch (err) { logger.error({ err }, "finance categories fetch error"); res.status(500).json({ error: "Error al cargar categorías" }); }
});
...
router.post("/finance/categories", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const [cat] = await db.insert(financeCategoriesTable).values({ userId, type, name, icon: icon ?? "circle", color: color ?? "#6b7280", isDefault: false }).returning();
    res.status(201).json(cat);
  } catch (err) { logger.error({ err }, "finance category create error"); res.status(500).json({ error: "Error al crear categoría" }); }
});
...
router.delete("/finance/categories/:id", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.json({ ok: true });
  } catch (err) { logger.error({ err }, "finance category delete error"); res.status(500).json({ error: "Error al eliminar" }); }
});
...
router.get("/finance/transactions", requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const [{ count }] = await db.select({ count: sql<number>`count(*)` }).from(financeTransactionsTable).where(and(...conditions));
    res.json({ transactions, total: Number(count) });
  } catch (err) { logger.error({ err }, "finance transactions fetch error"); res.status(500).json({ error: "Error al cargar movimientos" }); }
});
...
router.post("/finance/transactions", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  if (!type || !VALID_TX_TYPES.includes(type) || !amount || !date) { res.status(400).json({ error: "type, amount y date son requeridos" }); return; }
  try {
    ...
    if (accountId && (status === "confirmed" || !status)) {
      const [acct] = await db.select().from(financeAccountsTable).where(eq(financeAccountsTable.id, parseInt(accountId, 10)));
      if (acct) {
        const delta = type === "income" ? parseFloat(amount) : -parseFloat(amount);
        await db.update(financeAccountsTable).set({ amount: String(parseFloat(acct.amount ?? "0") + delta) }).where(eq(financeAccountsTable.id, acct.id));
      }
    }
    res.status(201).json(tx);
  } catch (err) { logger.error({ err }, "finance transaction create error"); res.status(500).json({ error: "Error al crear movimiento" }); }
});
...
router.put("/finance/transactions/:id", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.json(updated);
  } catch (err) { logger.error({ err }, "finance transaction update error"); res.status(500).json({ error: "Error al actualizar" }); }
});
...
router.delete("/finance/transactions/:id", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.json({ ok: true });
  } catch (err) { logger.error({ err }, "finance transaction delete error"); res.status(500).json({ error: "Error al eliminar" }); }
});
...
router.get("/finance/recurring-rules", requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json(rules);
  } catch (err) { logger.error({ err }, "finance recurring rules fetch error"); res.status(500).json({ error: "Error al cargar reglas" }); }
});
...
router.post("/finance/recurring-rules", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  if (!name || !type || !amount || !frequency || !VALID_FREQUENCIES.includes(frequency)) { res.status(400).json({ error: "Campos obligatorios faltantes" }); return; }
  try {
  ...
  } catch (err) { logger.error({ err }, "finance recurring rule create error"); res.status(500).json({ error: "Error al crear regla" }); }
});
...
router.put("/finance/recurring-rules/:id", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.json(updated);
  } catch (err) { logger.error({ err }, "finance recurring rule update error"); res.status(500).json({ error: "Error al actualizar" }); }
});
...
router.delete("/finance/recurring-rules/:id", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.json({ ok: true });
  } catch (err) { logger.error({ err }, "finance recurring rule delete error"); res.status(500).json({ error: "Error al eliminar" }); }
});
...
router.get("/finance/summary", requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const monthEnd = `${yr}-${mo}-31`;
    const next30 = addDays(today, 30);

    const [incomeRows, expenseRows, accounts, rules, cats] = await Promise.all([
      db.select().from(financeTransactionsTable).where(and(eq(financeTransactionsTable.userId, userId), eq(financeTransactionsTable.type, "income"), gte(financeTransactionsTable.date, monthStart), lte(financeTransactionsTable.date, monthEnd))),
      db.select().from(financeTransactionsTable).where(and(eq(financeTransactionsTable.userId, userId), eq(financeTransactionsTable.type, "expense"), gte(financeTransactionsTable.date, monthStart), lte(financeTransactionsTable.date, monthEnd))),
      db.select().from(financeAccountsTable).where(eq(financeAccountsTable.userId, userId)),
      db.select().from(financeRecurringRulesTable).where(and(eq(financeRecurringRulesTable.userId, userId), eq(financeRecurringRulesTable.isActive, true))).orderBy(financeRecurringRulesTable.nextDate),
      db.select().from(financeCategoriesTable).where(eq(financeCategoriesTable.userId, userId)),
    ]);

    const confirmedIncome = incomeRows.filter(t => t.status !== "cancelled").reduce((s, t) => s + parseFloat(t.amount), 0);
    const confirmedExpense = expenseRows.filter(t => t.status !== "cancelled").reduce((s, t) => s + parseFloat(t.amount), 0);
    ...
    const upcomingRecurrences = rules
      ...
      .map(r => ({ id: r.id, name: r.name, type: r.type, amount: parseFloat(r.amount), frequency: r.frequency, nextDate: r.nextDate, category: r.categoryId ? catMap[r.categoryId] ?? null : null }));

    const expectedExpenses = rules.filter(r => r.type === "expense" && r.nextDate && r.nextDate <= monthEnd && r.nextDate >= today).reduce((s, r) => s + parseFloat(r.amount), 0);
    const expectedIncome = rules.filter(r => r.type === "income" && r.nextDate && r.nextDate <= monthEnd && r.nextDate >= today).reduce((s, r) => s + parseFloat(r.amount), 0);
    const estimatedBalance = totalAssets - totalDebt + expectedIncome - expectedExpenses;

    const recent = await db.select().from(financeTransactionsTable)
      .where(eq(financeTransactionsTable.userId, userId))
      .orderBy(desc(financeTransactionsTable.date), desc(financeTransactionsTable.id)).limit(8);

    const recentWithCat = recent.map(t => ({ ...t, amount: parseFloat(t.amount), category: t.categoryId ? catMap[t.categoryId] ?? null : null }));

    const alerts: { level: "green" | "yellow" | "red"; message: string }[] = [];
    const savingsRate = confirmedIncome > 0 ? (confirmedIncome - confirmedExpense) / confirmedIncome : 0;
    if (confirmedIncome > 0 && savingsRate < 0) alerts.push({ level: "red", message: "Gastos superan ingresos del mes" });
    else if (confirmedIncome > 0 && savingsRate < 0.1) alerts.push({ level: "yellow", message: "Tasa de ahorro baja (menos del 10%)" });
    else if (confirmedIncome > 0) alerts.push({ level: "green", message: `Ahorro del mes: ${Math.round(savingsRate * 100)}% de ingresos` });
    if (totalDebt > 0 && totalDebt > totalAssets * 0.5) alerts.push({ level: "red", message: "Deuda supera el 50% de activos" });
    if (upcomingRecurrences.length > 0) alerts.push({ level: "yellow", message: `${upcomingRecurrences.length} movimiento${upcomingRecurrences.length > 1 ? "s" : ""} recurrente${upcomingRecurrences.length > 1 ? "s" : ""} en los próximos 30 días` });

    res.json({
      ingresosMes: confirmedIncome,
      gastosMes: confirmedExpense,
      saldoEstimadoFinMes: estimatedBalance,
...
router.get("/finance/accounts", requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json(accounts);
  } catch (err) { logger.error({ err }, "finance accounts fetch error"); res.status(500).json({ error: "Error al cargar cuentas" }); }
});
...
router.post("/finance/accounts", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  if (!type || !VALID_ACCOUNT_TYPES.includes(type as any) || !label) { res.status(400).json({ error: "type y label son requeridos" }); return; }
  try {
  ...
  } catch (err) { logger.error({ err }, "finance account create error"); res.status(500).json({ error: "Error al crear cuenta" }); }
});
...
router.put("/finance/accounts/:id", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.json(updated);
  } catch (err) { logger.error({ err }, "finance account update error"); res.status(500).json({ error: "Error al actualizar cuenta" }); }
});
...
router.delete("/finance/accounts/:id", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.json({ ok: true });
  } catch (err) { logger.error({ err }, "finance account delete error"); res.status(500).json({ error: "Error al eliminar cuenta" }); }
});
...
router.get("/finance/config", requireAuth, async (_req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json(configMap);
  } catch (err) { logger.error({ err }, "finance config fetch error"); res.status(500).json({ error: "Error al cargar configuración" }); }
});
...
router.put("/finance/config/:key", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.json({ key, value });
  } catch (err) { logger.error({ err }, "finance config update error"); res.status(500).json({ error: "Error al guardar configuración" }); }
});
...
router.post("/finance/seed-demo", requireAuth, async (req: Request, res: Response): Promise<void> => {
  const userId = getCurrentUserId(req);
  try {
    const existingTx = await db.select().from(financeTransactionsTable).where(eq(financeTransactionsTable.userId, userId)).limit(1);
    if (existingTx.length > 0) { res.json({ ok: true, skipped: true, message: "Ya hay datos cargados" }); return; }
    ...
    if (existingAccts.length === 0) {
      const [b] = await db.insert(financeAccountsTable).values({ userId, type: "banco", label: "Cuenta Bancaria Principal", amount: "850000", currency: "ARS" }).returning();
      const [bv] = await db.insert(financeAccountsTable).values({ userId, type: "billetera_virtual", label: "Mercado Pago", amount: "45000", currency: "ARS" }).returning();
    ...
    const d = (n: number, offset = 0) => {
      const mo = offset < 0 ? String(parseInt(tm, 10) + offset).padStart(2, "0") : tm;
      const yr2 = offset < 0 && parseInt(tm, 10) + offset <= 0 ? String(parseInt(ty, 10) - 1) : ty;
      const realMo = offset < 0 && parseInt(tm, 10) + offset <= 0 ? "12" : mo;
      return `${yr2}-${realMo}-${String(n).padStart(2, "0")}`;
    };
    ...
    await db.insert(financeTransactionsTable).values(tx.map(t => ({ ...t, userId, currency: "ARS" })));
    ...
    await db.insert(financeRecurringRulesTable).values(rules.map(r => ({ ...r, userId, currency: "ARS", isActive: true })));

    res.json({ ok: true, skipped: false, message: "Datos demo cargados exitosamente" });
  } catch (err) { logger.error({ err }, "finance seed demo error"); res.status(500).json({ error: "Error al cargar datos demo" }); }
});
