...
function loadSidebarState(): SidebarState {
  try { return (localStorage.getItem("sidebar-state") as SidebarState) ?? "expanded"; }
  catch { return "expanded"; }
}
function loadSidebarPinned(): boolean {
  try { return localStorage.getItem("sidebar-pinned") !== "false"; }
  catch { return true; }
}
...
// ── Nav items (constante de módulo — nunca se recrea) ─────────────────────────
...
function useSortableNavOrder(items: readonly NavItem[]) {
...
// ── NavLink (memoizado) ───────────────────────────────────────────────────────
// memo() previene re-renders cuando cambios de estado en el layout padre
// (como abrir el Sheet mobile o el ModoHoy) re-renderizan el sidebar.
// Con 22 NavLinks, cada render del sidebar sin memo equivale a 22 renders
// de NavLink aunque ninguno haya cambiado de activo/inactivo.
...
// ── SidebarContent (memoizado) ────────────────────────────────────────────────
// memo() evita que SidebarContent se re-renderice cuando el estado del layout
// padre cambia (modoHoyOpen, sheetOpen, refreshing). Esas variables no son
// props de SidebarContent — sin memo, el sidebar completo se re-renderiza
// incluyendo los 22 NavLinks, la query de módulos, y el DndContext.
    ...
    return (
      <div className="flex h-full flex-col bg-sidebar border-r border-sidebar-border items-center py-3 gap-1">
        ...
        <nav className="flex flex-col gap-1 flex-1 overflow-y-auto w-full items-center px-1">
          {sorted.map((item) => (
            <NavLink
              key={item.href} {...item} location={location}
              onClick={onNavigate} collapsed
...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const setSidebarState  = useCallback((s: SidebarState) => { setSidebarStateRaw(s); saveSidebarState(s); }, []);
  const setSidebarPinned = useCallback((p: boolean)       => { setSidebarPinnedRaw(p); saveSidebarPinned(p); }, []);
  ...
  // useCallback evita nueva referencia en cada render — el botón no se re-renderiza
  ...
  const openModoHoy   = useCallback(() => setModoHoyOpen(true),  []);
  const closeModoHoy  = useCallback(() => setModoHoyOpen(false), []);
  ...
  return (
    <div className="flex min-h-[100dvh] w-full overflow-x-hidden">
      ...
      >
        {/* SidebarContent está memoizado — solo re-renderiza cuando sus props cambian.
            Estados como modoHoyOpen, sheetOpen, refreshing NO son props del sidebar,
            por lo que sus cambios no lo afectan. */}
        <SidebarContent
              ...
              <DropdownMenuContent align="end" className="w-52">
                <DropdownMenuLabel>
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs font-normal text-muted-foreground truncate">{auth.userEmail}</span>
                  </div>
                </DropdownMenuLabel>
      ...
      <ModoHoy open={modoHoyOpen} onClose={closeModoHoy} />
    </div>
  );
}
